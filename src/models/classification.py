...
class HMSEEGClassifierModule(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        epsilon = 1e-9
        y_hat = y_hat.clamp(min=epsilon)  # Evitare zeri in y_hat
        vote_distr = vote_distr.clamp(min=epsilon)  # Evitare zeri in vote_distr
        kl_div_value = F.kl_div(y_hat.log(), vote_distr, reduction='batchmean')
        self.log('test_kl_div', kl_div_value, on_step=False, on_epoch=True, logger=True)
        self.log('test_kl_divergence', self.kl_divergence(y_hat, vote_distr), on_step=False, on_epoch=True, logger=True)
...
class HMSEEGClassifierWithProbabilities(LightningModule):
    ...
    def __init__(self, signal_len, num_classes, lr=1e-5, max_epochs=100):
        ...
        self.kl_div = torchmetrics.functional.kl_divergence
        self.accuracy = torchmetrics.Accuracy(task='multiclass', num_classes=num_classes)   
        self.recall = torchmetrics.Recall(task='multiclass', average='weighted', num_classes=num_classes)
        self.precision = torchmetrics.Precision(task='multiclass', average='weighted', num_classes=num_classes)
        self.f1 = torchmetrics.F1(task='multiclass', average='weighted', num_classes=num_classes)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Log delle metriche
        self.log('test_kl_div', self.kl_div(y_hat, labels), on_step=False, on_epoch=True, logger=True)
        self.log('test_accuracy', self.accuracy(predictions, labels), on_step=False, on_epoch=True, logger=True)
        self.log('test_recall', self.recall(predictions, labels), on_step=False, on_epoch=True, logger=True)
        self.log('test_precision', self.precision(predictions, labels), on_step=False, on_epoch=True, logger=True)
        self.log('test_f1', self.f1(predictions, labels), on_step=False, on_epoch=True, logger=True)
...
class HMSCWTClassifierWithProbabilities(LightningModule):

    def __init__(self, num_channels=20, n_freq=64, n_time=2000, num_classes=6, lr=1e-5, max_epochs=100):
        super().__init__()
        self.save_hyperparameters()
        ...
        # Calcolo della dimensione dell'input per il fully connected layer
        self.fc_input_size = 512 * (n_freq // 32) * (n_time // 32)
        self.fc1 = nn.Linear(self.fc_input_size, 128)
        self.fc2 = nn.Linear(128, num_classes)

        # Definizione delle funzioni di attivazione e di loss
        self.relu = nn.ReLU()
        self.softmax = nn.Softmax(dim=1)
        self.loss_fn = nn.KLDivLoss(reduction='batchmean')  # KL Divergence come funzione di perdita
    ...
    def _common_step(self, batch, batch_idx, stage):
        ...
        pred = self(images)
        # loss = self.loss(pred, labels)
        loss = self.loss(torch.log(pred), vote_distr)
        self.log(f"{stage}_loss", loss, on_step=False, on_epoch=True)
